  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-3xl px-5 py-12 text-paper/80 sm:px-8 sm:py-20">
        ...
        <p className="mt-4 text-sm">Atualizada em 18 de setembro de 2026.</p>
        <div className="break-words text-base leading-relaxed">
          ...
          <section className={sectionClass}>
            ...
            <p>Com sua permissão, o site carrega a tag do Google Ads para medir tentativas de contato pelo formulário ou botão de WhatsApp. A tag não é carregada antes de aceitar nem quando você recusa inicialmente. Essa medição não confirma envio de mensagem, atendimento ou contratação. O código não transmite os campos do formulário ou o endereço da mensagem do WhatsApp ao Google.</p>
            <p>A tag pode tratar identificadores de publicidade, cookies como _gcl_aw, endereço IP e informações técnicas do navegador. Esses dados são destinados ao Google para medição publicitária e podem ser processados fora do Brasil. Não habilitamos Google Analytics, personalização de anúncios ou conversões otimizadas nesta integração. Consulte também a <a href="https://policies.google.com/privacy?hl=pt-BR" className="text-gold-bright underline" target="_blank" rel="noopener noreferrer">política de privacidade do Google</a>.</p>
            <p>Sua escolha é conservada no armazenamento local por até 180 dias. Após aceitar, um indicador no armazenamento da sessão evita repetir o evento de contato na mesma visita. Os cookies de medição têm duração controlada pelo Google. Você pode mudar a escolha abaixo. Ao revogar, novos eventos de contato são bloqueados e a tag recebe a atualização para negar consentimento; o script já carregado permanece na página até uma nova navegação ou recarregamento. A revogação não apaga dados já recebidos pelo Google. Cookies existentes também podem ser removidos pelas configurações do navegador.</p>
            <CookiePreferences />
